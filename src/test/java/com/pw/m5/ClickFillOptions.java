package com.pw.m5;
import com.microsoft.playwright.Page;
import com.pw.ScriptBase;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;

public class ClickFillOptions  extends ScriptBase{

    @Test
    public void clickOptionsCount(){

        page.navigate(home);
        page.click("#clap-image", new Page.ClickOptions().setClickCount(2));

        Assertions.assertTrue(page.isVisible("#thank-you-2"));

    }
    @Test
    public void fillOptions(){
        page.navigate(home);
        page.fill("#exampleMessage", "So I was thinking the other day", new Page.FillOptions().setForce(true));
    }

    @Test
    public void checkScenario(){
        page.navigate(home);
        page.fill("#exampleFormControlInput1", "mail@example.com");
        page.click("#contactReason");
        page.selectOption("id=contactReason", "Bored");
        page.check("#sendCopy");
        page.uncheck("#sendCopy");
        page.fill("#exampleMessage", "bored....");
        page.click("#submit-contact");
        Assertions.assertTrue(page.isVisible("text=OK, but please make it interesting"));
    }
}
